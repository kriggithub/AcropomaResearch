...
## IRI Calculations
```{r}
# IRI = [# individs. with the item/# individs. with any item(s)] * [(# of a prey item / total # prey items) + (sum of volumes of a prey item / sum of volumes of all prey items)]


itemdata <- read.csv(file = "fishdat1sum.csv")
...
HManimalIRI <- ((nHManimalIndivid/nHMindivid) * ((nHManimalItems/nHMitems) + (volHManimal/volHMitems))) * 100
...
HMcrustaceanIRI <- ((nHMcrustaceanIndivid/nHMindivid) * ((nHMcrustaceanItems/nHMitems) + (volHMcrustacean/volHMitems))) * 100

# fish
nHMfishIndivid <- sum(hanMayCountSub$fishcount)
nHMfishItems <- sum(hanMayItemSub$Category == "fish")
volHMfish <- sum(hanMayCountSub$fish)
HMfishIRI <- ((nHMfishIndivid/nHMindivid) * ((nHMfishItems/nHMitems) + (volHMfish/volHMitems))) * 100
...
HMmolluskIRI <- ((nHMmolluskIndivid/nHMindivid) * ((nHMmolluskItems/nHMitems) + (volHMmollusk/volHMitems))) * 100
...
HFanimalIRI <- ((nHFanimalIndivid/nHFindivid) * ((nHFanimalItems/nHFitems) + (volHFanimal/volHFitems))) * 100
...
HFcrustaceanIRI <- ((nHFcrustaceanIndivid/nHFindivid) * ((nHFcrustaceanItems/nHFitems) + (volHFcrustacean/volHFitems))) * 100

# fish
nHFfishIndivid <- sum(hanFebCountSub$fishcount)
nHFfishItems <- sum(hanFebItemSub$Category == "fish")
volHFfish <- sum(hanFebCountSub$fish)
HFfishIRI <- ((nHFfishIndivid/nHFindivid) * ((nHFfishItems/nHFitems) + (volHFfish/volHFitems))) * 100
...
HFmolluskIRI <- ((nHFmolluskIndivid/nHFindivid) * ((nHFmolluskItems/nHFitems) + (volHFmollusk/volHFitems))) * 100
...
JFanimalIRI <- ((nJFanimalIndivid/nJFindivid) * ((nJFanimalItems/nJFitems) + (volJFanimal/volJFitems))) * 100
...
JFcrustaceanIRI <- ((nJFcrustaceanIndivid/nJFindivid) * ((nJFcrustaceanItems/nJFitems) + (volJFcrustacean/volJFitems))) * 100

# fish
nJFfishIndivid <- sum(japFebCountSub$fishcount)
nJFfishItems <- sum(japFebItemSub$Category == "fish")
volJFfish <- sum(japFebCountSub$fish)
JFfishIRI <- ((nJFfishIndivid/nJFindivid) * ((nJFfishItems/nJFitems) + (volJFfish/volJFitems))) * 100
...
JFmolluskIRI <- ((nJFmolluskIndivid/nJFindivid) * ((nJFmolluskItems/nJFitems) + (volJFmollusk/volJFitems))) * 100
...
JManimalIRI <- ((nJManimalIndivid/nJMindivid) * ((nJManimalItems/nJMitems) + (volJManimal/volJMitems))) * 100
...
JMcrustaceanIRI <- ((nJMcrustaceanIndivid/nJMindivid) * ((nJMcrustaceanItems/nJMitems) + (volJMcrustacean/volJMitems))) * 100

# fish
nJMfishIndivid <- sum(japMayCountSub$fishcount)
nJMfishItems <- sum(japMayItemSub$Category == "fish")
volJMfish <- sum(japMayCountSub$fish)
JMfishIRI <- ((nJMfishIndivid/nJMindivid) * ((nJMfishItems/nJMitems) + (volJMfish/volJMitems))) * 100
...
JMmolluskIRI <- ((nJMmolluskIndivid/nJMindivid) * ((nJMmolluskItems/nJMitems) + (volJMmollusk/volJMitems))) * 100
...
# Turn each tibble into a tableGrob
tab1 <- tableGrob(hmIRItable, rows = NULL)
tab2 <- tableGrob(hfIRItable, rows = NULL)
tab3 <- tableGrob(jfIRItable, rows = NULL)
tab4 <- tableGrob(jmIRItable, rows = NULL)

# Arrange them side by side (2x2 grid if easier to read, or 1x4 if wide is fine)
g <- grid.arrange(tab1, tab2, tab3, tab4, ncol = 2)

# Save as an image
ggsave("allIRItables.png", g, width = 10, height = 6)
